...
- Two interrupt-blocking mechanisms: `AudioNoInterrupts()`/`AudioInterrupts()` is an audio
  library mutex that blocks the audio update thread (use for audio parameter writes).
  `noInterrupts()`/`interrupts()` sets ARM PRIMASK to block all IRQs (use for ISR-shared
  variable snapshots). They serve different purposes — don't interchange them.
